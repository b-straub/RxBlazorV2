# RXBG042: Non-Observable Collection Type on Partial Property
...
Replace with the corresponding reactive collection type from ObservableCollections and use `private init` accessor:
...
[ObservableComponentTrigger]
public partial ObservableList<Email> Emails { get; private init; } = [];
```

Use `private init` instead of `set` because `ObservableList<T>` gets reactivity from observing the collection itself, not from property reassignment. The `private` modifier ensures the collection reference can only be set during initialization within the class.
