...
{
    public static void GenerateObservableModelPartials(SourceProductionContext context, ObservableModelInfo modelInfo, int updateFrequencyMs = 100)
    {
        try
        {
        ...
        sb.AppendLine();
        sb.AppendLine($"public partial class {modelInfo.ClassName}");
        sb.AppendLine("{");
        ...
        // Generate constructor if there are model references or DI fields
        if (modelInfo.ModelReferences.Any() || modelInfo.DIFields.Any())
        {
            ...
            if (modelInfo.ModelReferences.Any())
            {
                ...
                foreach (var modelRef in modelInfo.ModelReferences)
                {
                    ...
                    sb.AppendLine("            .Subscribe(chunks => StateHasChanged(chunks.SelectMany(c => c).ToArray())));");
                }
            }
    ...
    public static void GenerateAddObservableModelsExtension(SourceProductionContext context, ObservableModelInfo[] models)
    {
        try
        {
            var sb = new StringBuilder();
            ...
            foreach (var model in models)
            {
                ...
                if (model.ModelReferences.Any() || model.DIFields.Any())
                {
                    ...
                    sb.AppendLine($"        services.{registrationMethod}<{model.ClassName}>(sp => new {model.ClassName}({allResolvers}));");
                }
                else
                {
            ...
            context.AddSource("ObservableModelServiceExtensions.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
        }
        catch (Exception ex)
        {
            var diagnostic = Diagnostic.Create(
