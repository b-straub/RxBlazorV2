...
**CRITICAL (must fix):**
- `List<T>`, `IList<T>`, or `Collection<T>` on a partial property in an ObservableModel — Mutations (Add, Remove, Clear) are invisible to the reactive system. Use `ObservableList<T>` with `init` accessor so collection changes fire through the pipeline. Especially bad when combined with `[ObservableComponentTrigger]` — the trigger only fires on full property reassignment, not on item-level mutations, making it silently broken.
