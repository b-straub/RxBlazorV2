...
**CRITICAL (must fix):**
- `OnAfterRender` or `OnAfterRenderAsync` override in a `*ModelComponent` — Breaks the `ObservableComponent<T>` lifecycle. The base class wires the Observable subscriptions in `SetParametersAsync` (before the first render, so a change made by an earlier component's context-ready chain in the same render batch is not lost), then uses `OnAfterRender` to run `OnContextReady()`/`Model.ContextReady()` and `OnAfterRenderAsync` to run `Model.ContextReadyAsync()`. Overriding either without calling `base` silently skips the context-ready chain — models never initialize, referenced models never start, and the `Initialized` flag stays false. The same applies to a `SetParametersAsync` override that skips `base`: no subscriptions, no re-render, no trigger hooks. Move initialization logic to `OnContextReady`/`OnContextReadyAsync` (sync/async hooks called by the base class after setup completes). If you must override, **always call base first**.
- `StateHasChanged()` — Manual calls bypass reactive system. Use partial properties. Component triggers only generate hooks — rendering is handled by properties in razor.
...
- Manual `try { ... } catch (Exception ex) { ErrorMessage = $"Failed to X: {ex.Message}"; }` inside a command body solely to prefix the exception's message — Defeats the framework's automatic exception capture. Use the third positional argument of `[ObservableCommand]` to name a `string Format(Exception)` companion method on the model. The factory invokes the formatter, populates `Command.Error` (the exception) and `Command.ErrorMessage` (the formatted string), and forwards the formatted text to a configured `StatusBaseModel`. Pattern-match exception types inside the formatter (e.g. `OperationCanceledException` → "Cancelled.", `HttpRequestException` → ...). The cancelable factories already short-circuit `OperationCanceledException` before the formatter is called.
...
- `[Parameter] EventCallback ...` declared on a `*ModelComponent`-derived child to bubble UI events to a parent that inherits from the **same** (or related) `*ModelComponent` — Classic "EventCallback hell": both components share the Model via DI, so the child should not relay button clicks back up the component tree. This is the most common AI-generated fallback pattern when the model isn't recognized as the communication channel. Replace with one of: (a) **handle in child** — the child already has its own injected services (`IDialogService`, `NavigationManager`, etc.), so move the dialog/navigation/async call into the child; (b) **add a model command** — wrap the action in `[ObservableCommand]` on the shared model and bind via `<MudButtonRx Command="Model.X">` in the child (parent doesn't need to know); (c) **service-based dialog request** — inject an `IDialogPresenter` service into the model and call it from a command. EventCallback bubbling between reactive components defeats the reactive pipeline, creates fragile parent-child coupling, and bypasses CanExecute/Executing wiring.
- `OnClick="@(() => SomeCallback.InvokeAsync(...))"` in a `*ModelComponent`-derived child — Razor symptom of the EventCallback bubbling anti-pattern above. The button should bind to a model command via `<MudButtonRx Command="Model.X">` (or the relevant `*Rx`/`*AsyncRx` variant) so CanExecute, Executing state, and cancellation are wired automatically.
...
- Any `[Parameter] EventCallback`/`EventCallback<T>` on a reactive component (`*ModelComponent`-derived) where the parent could share or inject the same Model — Even if the handler does real work, the reactive way is shared model state. Audit the parent's handler: if it (a) shows a dialog, (b) navigates, (c) calls a service, the action belongs in a model command or a child-handled scope, not in event plumbing.
